		...
		while (res >= 1) {
			...
			if (strcmp(lineHeader, "v") == 0) {
				...
				if (matches == 9) {
					...
					matches = fscanf(file, "%d%d%d", &vertexIndex[1], &vertexIndex[2], &vertexIndex[3]) + 1;
					...
					for (int i = 0; i < matches; ++i) {
						if (vertexIndex[i] < 0)
							vertexIndex[i] = mesh->vertices.size() / 3 + vertexIndex[i];
					}
				}
